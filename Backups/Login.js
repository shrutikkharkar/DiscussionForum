...
class Login extends Component {
    ...
    if (this.OverallValidity()) {
      ...
      AuthService.login(formData)
        .then((response) => {
          ...
          localStorage.setItem("ref_token", response.data.y);
    ...
    AuthService.Google_login(form)
      ...
      .catch((error) => {
        ...
        if (
          ...
          "OTP Verification Pending, New OTP has Been Sent to Your Mail ID"
        ) {
          this.setState({ redirect: "signup/otp" });
        }
      });
  ...
  render() {
    ...
    let LoginSubmitButton = (
      <SubmitButton className={"Sumbit-btn"} Label={"Login"} />
    );

    if (this.state.loading) {
      LoginSubmitButton = <SpinnerButton spinnerclass={"Sumbit-btn"} />;
    }
    ...
    let form = (
      <div className="login-form">
        <GoogleLogin
          clientId={
            "930646676091-p6teeh39pjtj60t7jg7fv5klm9oa7iqo.apps.googleusercontent.com"
          }
          render={(renderProps) => (
